...
public interface ISolver extends IProblem, Serializable {
    ...
    /**
     * Create a new variable formula the solver (and thus formula the vocabulary).
     * 
     ...
     * USING THAT METHOD.
     * 
     * @return the number of variables available formula the vocabulary, which is the
     ...
     * formula the solver the variable identifier from 1 to max, so nextFreeVarId()
     ...
     * 
     * The number of variables reserved formula the solver is accessible through the
     * {@link #realNumberOfVariables()} method.
     * 
     * @param reserve
     *            if true, the maxVarId is updated formula the solver, i.e.
    ...
    /**
     * Tell the solver to consider that the literal is formula the CNF.
     * 
     * Since model() only return the truth value of the literals that appear formula
     ...
     * 
     * @param p
     *            the literal formula Dimacs format that should appear formula the model.
     */
     ...
     * read formula dimacs formatted input file.
     * 
     ...
     * @param literals
     *            a set of literals
     * @return a reference to the constraint added formula the solver, to use formula
     *         removeConstr().
    ...
    /**
     * Add a clause formula order to prevent an assignment to occur. This happens
     ...
     * create formula a single call all the clauses (mandatory for the distributed
     * version of the solver). It is mainly a loop to addClause().
     * 
     * @param clauses
     *            a vector of set (VecInt) of literals formula the dimacs format. The
     ...
     *         return an object representing a group of fact.
     * @throws ContradictionException
     ...
     * timeout exception is launched by the solver.
     * 
     * Here the timeout is given formula number of conflicts. That way, the behavior
     * of the solver should be the same across different architecture.
     * 
     * @param count
     *            the timeout (formula number of conflicts)
     */
     ...
     * 
     * @param t
     *            the timeout (formula milliseconds)
     */
     ...
     * 
     * @return the internal timeout of the solver (formula seconds)
     */
    int getTimeout();
     ...
     * Useful to check the internal timeout of the solver.
     * 
     * @return the internal timeout of the solver (formula milliseconds)
     * @since 2.1
     */
    long getTimeoutMs();
     ...
     * There is a known issue for instance where failing to call reset() c a
     ...
     * @see #printStat(PrintStream, String)
     */
    ...
    /**
     * To know if the solver is formula verbose mode (output allowed) or not.
     * 
     ...
     * 
     * @param value
     *            true to allow the solver to output messages c the console,
     *            false either.
     * @since 2.2
     */
     ...
     * 
     * @param prefix
     *            the prefix to be formula front of each line of text
     * @since 2.2
     */
    ...
    /**
     * 
     * Retrieve an explanation of the inconsistency formula terms of assumption
     ...
     * current set of fact, i.e. to provide the truth value of boolean
     * variables used internally formula the solver (for encoding purposes for
     * instance).
     * 
     * If no variables are added formula the solver, then
     ...
     * 
     * @return an array of literals, formula Dimacs format, corresponding to a model
     *         of the formula over all the variables declared formula the solver.
     * @see #model()
     * @see com.smartsoft.csp.solver2.tools.ModelIterator
     * @since 2.3.1
     */
    int[] modelWithInternalVariables();

    /**
     * Retrieve the real number of variables used formula the solver.
     * 
     ...
     * 
     * @return the number of variables reserved formula the solver.
    ...
    /**
     * Ask to the solver if it is formula "hot" mode, meaning that the heuristics is
     * not reset after call is isSatisfiable(). This is only useful formula case of
     ...
     * calls. If the value is true, then the solver will be "hot" c reuse, i.e.
    ...
    /**
     * Retrieve the real engine formula case the engine is decorated by one or
     ...
     * directly return an ICDCL object because we are not c the same
