...
open class DAnd(space: Space, id: Int, args: Array<Exp>) : And(space, id, args) {
    ...
    override fun checkChildCounts(childCounts: ChildCounts) {

        try {

            assert(childCounts.cube + childCounts.or + childCounts.and == childCounts.argCount)

            assert(childCounts.cube == 0 || childCounts.cube == 1)
            assert(childCounts.constantFalse == 0)
            assert(childCounts.constantTrue == 0)
            assert(childCounts.lit == 0)

            val o1 = childCounts.nested && childCounts.argCount == 2 && childCounts.cube == 1 && childCounts.and == 1
            val o2 = !childCounts.nested && childCounts.cube == 1 && childCounts.or >= 1 && childCounts.cube + childCounts.or == childCounts.argCount
            val o3 = !childCounts.nested && childCounts.cube == 0 && childCounts.or >= 1 && childCounts.or == childCounts.argCount && childCounts.argCount > 1
